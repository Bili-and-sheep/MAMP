...
class Estatik
{
    ...
    public function init()
    {
        ...
        if ( is_admin() ) {
            ...
            if ( isset( $_GET['post_type'] ) && $_GET['post_type'] == Es_Property::get_post_type_name() ) {
                Es_Property_List_Page::init();
            }

            // Initialize update pro page.
            if ( isset( $_GET['page'] ) && $_GET['page'] == 'es_pro' ) {
                Es_Upgrade_Pro_Page::init();
            }

	        if ( isset( $_GET['page'] ) && ( $_GET['page'] == 'es_agent' || $_GET['page'] == 'es_buyer' ) ) {
			...
			if ( $user_entity ) {
				if ( $user_entity::STATUS_DISABLED == $user_entity->status ) {
					...
					wp_redirect( ! empty( $_POST['redirect_to'] ) && $user->has_cap( 'delete_es_properties' )
						? $_POST['redirect_to'] : home_url() );
					die;
				}
			} else {
				wp_redirect( ! empty( $_POST['redirect'] ) ? $_POST['redirect'] : $_POST['_wp_http_referer'] );
				die;
			}
		}
	}
	...
	public function login()
	{
		...
		if ( ! empty( $_REQUEST[ $nonce ] ) && wp_verify_nonce( $_REQUEST[ $nonce ], $nonce ) ) {
			...
			if ( is_wp_error( $user ) ) {
				...
				wp_redirect( ! empty( $_POST['redirect'] ) ? $_POST['redirect'] : $_POST['_wp_http_referer'] ); die;
			}
		}
	}
    ...
    public function _migration()
    {
	    ...
	    if ( ! get_option( 'es_role_permissions' ) ) {
	    	...
	    	if ( ! $role->has_cap( 'es_view_calculated_units' ) ) {
			    $role->add_cap( 'es_view_calculated_units' );
		    }

		    if ( ! $role->has_cap( 'es_delete_dm_item' ) ) {
			    $role->add_cap( 'es_delete_dm_item' );
		    }

		    if ( ! $role->has_cap( 'es_save_dm_item' ) ) {
			    $role->add_cap( 'es_save_dm_item' );
		    }

		    if ( ! $role->has_cap( 'es_manage_fb' ) ) {
			    $role->add_cap( 'es_manage_fb' );
		    }
	    }
    }
}
