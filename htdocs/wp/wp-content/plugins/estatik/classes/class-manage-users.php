...
class Es_Manage_Users extends Es_Object
{
		...
		if ( ! empty( $_POST[ $nonce ] ) && wp_verify_nonce( $_POST[ $nonce ], $nonce ) && ! empty( $_POST[ 'es_user' ] ) ) {
				...
				if ( ! empty( $_POST['g-recaptcha-response'] ) ) {
					...
					if ( ! empty( $verifyResponse['body'] ) ) {
						...
						if ( ! empty( $responseData->success ) ) {
							...
							$logger->set_message( __( 'Captcha validation failed.', 'es-plugin' ), 'error' );
							$valid_request = false;
						}
					} else {
						$logger->set_message( __( 'Can\'t connect to google services. Please, contact the support.', 'es-plugin' ), 'error' );
						$valid_request = false;
					}
				} else {
			...
			if ( $valid_request ) {
			    ...
			    if ( in_array( $role, $valid_roles ) ) {
				    ...
				    // Register new agent.
				    $registered = $user->register( $_POST[ 'es_user' ] );
	...
	{
		$nonce = 'es-restore-pwd';

		if ( ! empty( $_REQUEST[ $nonce ] ) && wp_verify_nonce( $_REQUEST[ $nonce ], $nonce ) ) {
			$messenger = new Es_Messenger( 'login' );

			if ( ! empty( $_POST['action'] ) && ! empty( $_POST['_key'] ) && ! empty( $_POST['_login'] ) && $_POST['action'] == 'fill_password' ) {
				$user = check_password_reset_key( $_POST['_key'], $_POST['_login'] );
				...
				add_filter( 'wp_die_handler', function() use ( $messenger ) {
					...
					wp_redirect( $_POST['redirect'] ); die;
				} );
