...
class Es_User_Profile
{
	...
	{
		if ( ! empty( $_GET['es-delete-user'] ) && wp_verify_nonce( $_GET['es-delete-user'], 'es-delete-user' ) ) {
	...
	public static function render()
	{
		...
		$id = ! empty( $_GET['id'] ) && is_string( $_GET['id'] ) ? $_GET['id'] : 0;
		$id = intval( $id );
	...
	{
		if ( ! empty( $_POST['es_save_user'] ) && wp_verify_nonce( $_POST['es_save_user'], 'es_save_user' ) ) {
			...
			$_POST['nickname'] = $_POST['user_login'];

			// Get user ID.
			$id = ! empty( $_POST['user_id'] ) ? $_POST['user_id'] : 0;
