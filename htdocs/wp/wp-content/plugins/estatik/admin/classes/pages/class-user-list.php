...
class Es_User_List_Page
{
	...
	{
		$action = ! empty( $_GET['es-action'] ) ? $_GET['es-action'] : null;
...
//				$meta_query[] = array( 'key' => 'es_agent_name', 'value' => $filter['name'] );
	...
	{
		if ( ! empty( $_GET['role'] ) && $_GET['role'] == Es_Buyer::get_role_name() ) {
	...
	public function row_actions( $actions, $user )
	{
		...
		$actions['delete'] = "<a class='es-delete-user' href='" . wp_nonce_url( "admin.php?page=es_" . $user_entity->get_entity_name() . "&delete=" . $user->ID, 'es-delete-user', 'es-delete-user' ) . "'>
	...
	{
		return is_admin() && ! empty( $_GET['role'] ) && in_array( $_GET['role'], es_get_plugin_user_roles() );
	}
