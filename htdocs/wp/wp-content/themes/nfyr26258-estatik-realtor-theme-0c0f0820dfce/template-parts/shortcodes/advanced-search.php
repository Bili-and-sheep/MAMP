...
<form action="<?php echo $handler; ?>" method="GET" class="ert-search" style="max-width: <?php echo $atts['max-width']; ?>; padding: <?php echo $atts['padding']; ?>; width: <?php echo $atts['width']; ?>">
    ...
    <div class="row advanced-search-control">
        <div class="col-md-4">
            <a href="#" class="advanced-search-link js-advanced-search-link"><i class="fa fa-plus"></i><?php _e( 'Advanced Search', 'es-plugin' ); ?></a>
        </div>
        <div class="col-md-8">
            <div class="form-row">
                <div class="col ert-btn__wrap">
                    <input type="reset" class="btn btn-light form-control" value="<?php echo __( 'Reset Search', 'es-plugin' ); ?>">
                </div>
                <div class="col ert-btn__wrap">
                    <input type="submit" class="btn btn-primary form-control" value="<?php echo __( 'Search', 'es-plugin' ); ?>">
                </div>
	            <?php if ( ! empty( $atts['save_search_button'] ) ) : ?>
                    <div class="col ert-btn__wrap">
			            <?php if ( get_current_user_id() ) : ?>
                            <input type="button" class="btn btn-secondary js-es-save-search form-control" value="<?php _e( 'Save search', 'es-plugin' ); ?>"/>
			            <?php else: ?>
                            <input type="button" class="btn btn-secondary js-es-login-form form-control" value="<?php _e( 'Save search', 'es-plugin' ); ?>"/>
			            <?php endif; ?>
                    </div>
	            <?php endif; ?>
            </div>
