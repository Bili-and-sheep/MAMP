...
add_action( 'ert_archive_sorting_dropdown', 'ert_archive_sorting_dropdown' );
...
if ( ! function_exists( 'ert_archive_sorting_dropdown' ) ) {
	 ...
	 * @return void
	 */
	function ert_archive_sorting_dropdown() {
		es_load_template( 'property/filter.php' );
	}
}
	...
	function es_property_labels_badges() {
		...
		foreach ( $es_property->get_labels_list() as $label ) : ?>
			<?php if ( $value = $es_property->{ $label->slug } && ( $link = get_term_link( $label->term_id ) ) ) : ?>
				<?php if ( ! $link instanceof WP_Error ) : ?>
					<a class="badge badge-light badge-<?php echo $label->slug; ?>" target="_blank" href="<?php echo $link; ?>"><?php _e( $label->name, 'es-plugin' ) ; ?></a>
				<?php endif; ?>
			<?php endif; ?>
		<?php endforeach;
	}
