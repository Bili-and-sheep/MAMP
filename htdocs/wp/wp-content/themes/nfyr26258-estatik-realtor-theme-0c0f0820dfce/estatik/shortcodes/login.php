		...
		<form action="" method="post">
            ...
            <div class="form-group text-right forgot-pwd">
	            <?php if ( $es_settings->reset_password_page_id && ( get_post( $es_settings->reset_password_page_id ) ) ) : ?>
                    <a href="<?php echo get_the_permalink( $es_settings->reset_password_page_id ); ?>"><?php _e( 'Forgot a Password?', 'es-plugin' ); ?></a><br/>
	            <?php endif; ?>
            </div>
            ...
            <div class="form-group">
                <input type="submit" class="btn btn-primary" value="<?php _e( 'Sign In', 'es-plugin' ); ?>"/>
            </div>
		</form>
	</div>
<?php else: ?>
