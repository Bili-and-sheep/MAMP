...
function ert_register_widgets() {
	...
	if ( class_exists( 'SiteOrigin_Widget' ) ) {
		...
		if ( function_exists( 'es_get_property' ) ) {
			...
			require_once 'inc/widgets/class-taxonomy-widget.php';
...
function ert_admin_menu() {
	...
	add_menu_page(
		...
		'ert-site-options',
		'ert_admin_page',
		get_stylesheet_directory_uri() . '/assets/images/estatik.svg',
		30
	);
}
...
function ert_enqueue_scripts() {
	...
	// Theme main styles.
	wp_enqueue_style( 'ert-theme', get_stylesheet_directory_uri() . '/assets/css/style-min.css', array( 'bootstrap' ) );
	...
	wp_register_script( 'waypoints', 'http://cdnjs.cloudflare.com/ajax/libs/waypoints/2.0.3/waypoints.min.js', array( 'jquery' ) );
	wp_register_script( 'counterup', get_stylesheet_directory_uri() . '/assets/js/jquery.counterup.min.js', array( 'jquery', 'waypoints' ) );
	...
	wp_enqueue_script( 'ert-theme', get_stylesheet_directory_uri() . '/assets/js/scripts-min.js', $deps );
	...
	.widget.es-widget .es-request-widget-wrap input.btn[type=submit], .single-properties .es-request-widget-wrap input.btn[type=submit], .es_calc_form input.es_calc_submit {
        border: .75px solid $theme_color;
        color: $theme_color;
	}
	
	..es_calc_form input.es_calc_submit:hover, .es-btn.es-btn-blue-bordered:hover,#header .navbar-nav.ert-login-menu li:last-child a:hover, .ert-link-btn:hover, .btn.btn-light:hover, a.btn.btn-light:hover, #searchsubmit, .single-properties .es-request-widget-wrap input.btn[type=submit]:hover {
    ...
    .ert-post-item .ert-post-item__content, .ert-post-item .ert-property-item__content, .ert-property-item .ert-post-item__content, .ert-property-item .ert-property-item__content {
        border-bottom: .75px solid {$theme_color};
    }
...
function ert_property_item_fields() { ?>
	<div class="row ert-property-item__fields">
		...
		<?php es_the_formatted_area( '<div class="col-auto">' . __( 'Area', 'es-plugin' ) . ': ', '</div>' ); ?>
	</div>
<?php }
...
add_filter( 'es_property_sections', 'ert_property_sections' );
...
add_action( 'ert_single_amenities', 'ert_single_amenities' );
...
function ert_get_agent_social( $agent ) {
	...
	return array(
		'facebook-logo' => 'http://facebook.com',
 ...
 */
function ert_display_social_block( $data, $use_wrapper = true ) {
...
function ert_recommended_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'      => __( 'Estatik Real Estate', 'ert' ),
			'slug'      => 'estatik',
...
function ert_property_query_atts() {
	...
	$atts = array( 'sort' => 'recent', 'limit' => 10, 'prop_id' => null, 'address' => '', 'price_min' => '', 'price_max' => '' );
...
function ert_property_slider( $atts = array() ) {
	...
	$listings = new Es_My_Listing_Shortcode();
	$query_args = $listings->build_query_args( $atts );
...
function ert_button_shortcode( $atts = array() ) {
    ...
    return "<a href='" . esc_url( $atts['link'] ) . "' class='btn btn-" . $atts['type'] . "'>" . $atts['name'] . "</a>";
}
